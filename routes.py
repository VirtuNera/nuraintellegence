...
def start_quiz(topic_id, difficulty=None):
    ...
    if not difficulty:
        ...
        preferred_difficulties = ['Medium', 'Easy', 'Hard', 'Very Easy', 'Very Hard']
        selected_set = None
    ...
    quiz_data = {
        ...
        'total_marks': sum(q.marks_worth for q in questions),
...
def submit_quiz():
    ...
    results = process_quiz_submission_direct(learner.student_id, quiz_data, answers)
    
    # Generate AI feedback (only if submission was successful)
    ai_feedback = None
    if results and not results.get('error'):
        ai_feedback = nura_ai.generate_quiz_feedback(learner.student_id, results)
    else:
        ai_feedback = {"feedback": "Unable to generate feedback due to submission error.", "recommendations": []}
...
def process_quiz_submission_direct(student_id, quiz_data, answers):
    ...
    try:
        ...
        quiz_engine._update_performance_trends(student_id, quiz.topic_id, quiz.score)
...
def get_learner_performance_data(student_id):
    ...
    # Get recent quizzes
    recent_quizzes = Quiz.query.filter_by(student_id=student_id).order_by(Quiz.date_taken.desc()).limit(10).all()
    ...
    for subject in subject_proficiency:
        ...
        subject_proficiency[subject] = sum(scores) / len(scores) if scores else 0
    
    return {
        'recent_quizzes': [{'topic': q.topic.name, 'score': q.score, 'date': q.date_taken.strftime('%Y-%m-%d')} for q in recent_quizzes],
        'subject_proficiency': subject_proficiency,
        'trends': [{'topic': t.topic.name, 'score': t.proficiency_score, 'data': t.trend_graph_data} for t in trends]
    }
...
def get_class_overview_data():
    ...
    for subject in subjects:
        quizzes = Quiz.query.join(Topic).filter(Topic.subject_id == subject.subject_id).all()
        if quizzes:
...
def get_learners_traffic_light_data():
    ...
    for learner in learners:
        ...
        recent_quizzes = Quiz.query.filter_by(student_id=learner.student_id).order_by(Quiz.date_taken.desc()).limit(5).all()
        ...
        traffic_light_data.append({
            ...
            'status': status,
            'avg_score': round(avg_score, 2) if recent_quizzes else 0
        })
...
def get_admin_analytics():
    ...
    active_users_30d = Quiz.query.filter(Quiz.date_taken >= thirty_days_ago).distinct(Quiz.student_id).count()
    active_users_7d = Quiz.query.filter(Quiz.date_taken >= seven_days_ago).distinct(Quiz.student_id).count()
    ...
    completion_rate = (Quiz.query.filter(Quiz.score >= 70).count() / max(total_quizzes, 1)) * 100
    
    # Recent activity (last 7 days)
    recent_quizzes = Quiz.query.filter(Quiz.date_taken >= seven_days_ago).count()
    recent_users = Quiz.query.filter(Quiz.date_taken >= seven_days_ago).distinct(Quiz.student_id).count()
    ...
    for i in range(7):
        ...
        daily_quizzes = Quiz.query.filter(
            Quiz.date_taken >= day_start,
            Quiz.date_taken < day_end
        ).count()
    ...
    # Performance trends
    performance_trends = PerformanceTrend.query.order_by(PerformanceTrend.last_updated.desc()).limit(30).all()
        ...
        for subject in subjects:
            if subject.topics:
                ...
                for topic in subject.topics:
                    ...
                    if quizzes:
                        avg_score = sum(quiz.score for quiz in quizzes) / len(quizzes)
                        if avg_score >= 80:  # Consider 80% as completed
                            completed_topics += 1
                
                subject_progress[subject.subject_id] = round((completed_topics / total_topics) * 100, 1) if total_topics > 0 else 0
...
def learning_roadmap():
    ...
    try:
        ...
        for subject in subjects:
            ...
            for topic in subject.topics:
                ...
                if quizzes:
                    ...
                    total_possible = sum(quiz.total_marks for quiz in quizzes)
                    if total_possible > 0:
                        progress = round((sum(quiz.score for quiz in quizzes) / total_possible) * 100, 1)
                    else:
                        progress = 0
            ...
            # Calculate subject progress
            subject_progress = round((subject_completed / len(subject.topics)) * 100, 1) if subject.topics else 0
        ...
        # Calculate overall progress
        overall_progress = round(total_progress / len(subjects), 1) if subjects else 0
        ...
        badges = [
            {
                ...
                'icon': 'medal',
                'earned': completed_topics > 0,
                'earned_date': 'Recently' if completed_topics > 0 else None,
                'progress': min(100, completed_topics * 100) if completed_topics > 0 else 0
            },
            {
                'name': 'Topic Master',
...
def ai_support():
    ...
    try:
        ...
        if quizzes:
            ...
            total_score = sum(quiz.score for quiz in quizzes)
            total_possible = sum(quiz.total_marks for quiz in quizzes)
            overall_accuracy = round((total_score / total_possible) * 100, 1) if total_possible > 0 else 0
            
            # Calculate average time per question
            total_time = sum(quiz.time_taken for quiz in quizzes if quiz.time_taken)
            ...
            timeline_labels = [f"Quiz {i+1}" for i in range(len(recent_quizzes))]
            timeline_data = [round((quiz.score / quiz.total_marks) * 100, 1) for quiz in recent_quizzes]
            ...
            topic_data = [round((topic_accuracy[topic]['score'] / topic_accuracy[topic]['total']) * 100, 1) 
...
def quiz_history():
    ...
    try:
        ...
        # Get quiz history
        quizzes = Quiz.query.filter_by(student_id=learner.student_id).order_by(Quiz.date_taken.desc()).all()
...
def api_quiz_details(quiz_id):
    ...
    try:
        ...
        # Get quiz
        quiz = Quiz.query.filter_by(quiz_id=quiz_id, student_id=learner.student_id).first()
        if not quiz:
        ...
        quiz_data = {
            ...
            'difficulty': quiz.question_set.difficulty_level,
            'score': quiz.score,
            'total_marks': quiz.total_marks,
            'percentage': round((quiz.score / quiz.total_marks) * 100, 1),
            'date': quiz.date_taken.strftime('%Y-%m-%d %H:%M'),
            'time_taken': f"{quiz.time_taken // 60}m {quiz.time_taken % 60}s" if quiz.time_taken else 'N/A'
        }
